...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvAnon:
        return BitcoinGUI::tr("Received anon");
    case SendAnon:
        return BitcoinGUI::tr("Sent anon");
    case Other:
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredBtc, nCredAnon;
    wtx.GetCredit(nCredBtc, nCredAnon, true);
    int64_t nCredit = nCredBtc + nCredAnon;
    ...
    {
        if (nNet > 0 && nCredAnon > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvAnon, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendAnon, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvAnon;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv anon
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendAnon;
                    sub.address = CBitcoinAddress(ckidD).ToString();
