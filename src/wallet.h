...
public:
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nBtc, int64_t& nAnon) const
    {
        nBtc = 0;
        nAnon = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    bool GetCredit(int64_t& nCredBtc, int64_t& nCredAnon, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredBtc = nCredAnon = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredBtcCached = 0;
            nCredAnonCached = 0;
            pwallet->GetCredit(*this, nCredBtcCached, nCredAnonCached);
