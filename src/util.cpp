...
#else
    const char* pszModule = "HappinessIndex";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HappinessIndex
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HappinessIndex
    // Mac: ~/Library/Application Support/HappinessIndex
    // Unix: ~/.HappinessIndex
    
    
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HappinessIndex";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "HappinessIndex.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No HappinessIndex.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override HappinessIndex.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "HappinessIndexd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong HappinessIndex will not work properly.");
                    strMiscWarning = strMessage;
                    LogPrintf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("HappinessIndex"), CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING);
                }
            }
        }
        if (fDebug) {
